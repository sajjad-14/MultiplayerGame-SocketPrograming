...
global ping_time 
def receiveMove():
    global pos, start, user, ping_time
    while True:
        if isCon and start and user:
            try:
                ...
                timerecv = data[1]
                data = data[0]
                ping_time = round((time.time() - float(timerecv)) * 1000, 2)  # Calculate ping in milliseconds
                
                lblPingValue = Label(rightFrame1, font=('arial', 20, 'bold'), text=str(ping_time), padx=2, pady=2, bg="Cadet Blue", width=10)
                lblPingValue.grid(row=2, column=1, sticky=W)
                if data != "":
...
#ping_time = 0  # Global variable to store ping time
...
# Add a label for displaying ping time
lblPingText = Label(rightFrame1, font=('arial', 20, 'bold'), text="Ping:", padx=2, pady=2, bg="Cadet Blue")
lblPingText.grid(row=2, column=0, sticky=W)

lblPingValue = Label(rightFrame1, font=('arial', 20, 'bold'), text="0.00 ms", padx=2, pady=2, bg="Cadet Blue", width=10)
lblPingValue.grid(row=2, column=1, sticky=W)
